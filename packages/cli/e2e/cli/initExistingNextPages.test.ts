...
describe(
  'init in existing next-pages project without src',
  {
    timeout: 60000, // 1 min
  },
  () => {
    let tempDir: string
    const projectName = 'next-pages-no-src'
    ...
    it('should create configs and global css file', async () => {
      ...
      const pagesDirPath = path.join(tempDir, 'pages')
      const pagesDirExists = await pathExists(pagesDirPath)
      expect(pagesDirExists).toBe(true)

      const globalCssPath = path.join(tempDir, 'styles/globals.css')
