...
export const scaffoldNewProject = async (options: InitOptions, projectPrompts: PromptForNewProject): Promise<{ projectPath: string }> => {
	...
	const createProjectSpinner = spinner(`Creating a new ${framework == "next-app" ? txt.info("Next.js") : txt.info("Vite")} project. This might take some time.`).start()

	if (framework === "next-app") {
		await execa(
			"npx",
			["create-next-app", "--tailwind", "--eslint", "--typescript", "--app", projectName, `--use-${packageManager}`, useSrcDir ? "--src-dir" : "--no-src-dir", "--yes"],
