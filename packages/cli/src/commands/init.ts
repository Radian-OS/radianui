...
export const createUtils = async (projectDir: string, hasSrcDir: boolean, framework: FrameworkName) => {
  try {
    const baseDir =
      framework === 'vite' ? path.join(projectDir, 'src') : hasSrcDir ? path.join(projectDir, 'src') : projectDir
...
export const executeInit = async (options: InitOptions) => {
  ...
  if (!hasExistingProject) {
    ...
    await updateGlobalCssVariables(
      projectPath,
      projectPrompts.useSrcDir,
      projectPrompts.framework,
      projectPrompts.brandColor,
      projectPrompts.font
    )
  } else {
    ...
    logger.warn(
      `${txt.bold('Note:')} This will replace your global CSS file and add Radian OS styles and colors to your project`
    )
