...
const checkComponentsAvailability = async (
	...
	const invalidComponents = components.filter(
		(component) => !availableComponents.includes(component)
	)
 ...
 */
export async function addComponentsToProject(
	...
	for (const component of resolvedComponents) {
		...
		for (const file of component.files) {
			...
			if (fileExists && !options.overwrite) {
				...
				const isBlockDependency =
					blockDependencies.has(component.name) && component.type !== "block"
	...
	if (filesSkipped.length) {
		spinner(
			`Skipped ${filesSkipped.length} ${filesSkipped.length === 1 ? "file" : "files"}: (files might be identical,use --overwrite flag to overwrite)`
		)?.info()
...
const createFile = async (
	...
	let transformedContent = await transform(
		projectInfo,
		filePath,
		file.content,
		config
