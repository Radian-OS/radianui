...
function HoverCardContent({ className, sideOffset = 6, children, ...props }: HoverCardContentProps) {
	...
	return (
		<HoverCardPrimitive.Content
			...
			className={cn(
				"bg-base text-fgdata-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 outline-hidden z-50 w-64 rounded-md border p-4 shadow-md",
				className
			)}
			{...props}>
			{children}
