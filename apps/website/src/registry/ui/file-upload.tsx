      ...
      {((value.length == 0 && !multiple) || multiple) && (
        ...
        >
          <input
            type="file"
            className={cn(
              "absolute inset-0 z-40 h-full w-full cursor-pointer opacity-0",
          ...
          <div className="flex flex-col items-center justify-center gap-4">
            <span
              className={cn(
                "border-stroke bg-bg1 size-10 rounded-lg border p-2 drop-shadow-xs",
                { "border-error": error },
              )}
            >
              <Upload
...
function FilePreview({
  ...
  return (
    <div
      className={cn(
        "bg-bg3 flex flex-col items-center justify-start gap-3 rounded-lg p-3",
        { "bg-error/5": status?.status === "error" },
      )}
    >
      <div>
          ...
          <div className="w-full">
            <div className="flex w-full justify-between gap-1">
              ...
              <button
                type="button"
                onClick={onRemove}
                className="bg-bg1 size-5 rounded-full p-1"
              >
            ...
            <div className="body-xs text-fg2 flex gap-2 font-normal">
              ...
              {status && status.status === "error" && (
                <span className="text-error">Error while uploading</span>
              )}
            </div>
          </div>
        </div>
      ...
      {status && (
        <ProgressBar
          value={parseInt(status.progress.toString())}
          rootClassName="h-1"
        />
      )}
    </div>
