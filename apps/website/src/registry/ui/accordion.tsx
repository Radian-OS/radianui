...
function AccordionItem({ children, className, ...props }: AccordionItemProps) {
  ...
  const borderClass =
    variant === "closed"
      ? "mb-2 rounded-md border last:mb-0"
      : "border-b last:border-b-0";

  return (
    <AccordionPrimitive.Item
      data-slot="accordion-item"
      className={classNames("overflow-hidden", borderClass, className)}
      {...props}
    >
      {children}
    </AccordionPrimitive.Item>
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      ...
      >
        {children}
        <Plus
          className="text-fg2 size-5 shrink-0 group-data-[state=open]:hidden"
          aria-hidden
        />
        <Minus
          className="text-fg2 size-5 shrink-0 group-data-[state=closed]:hidden"
          aria-hidden
        />
      </AccordionPrimitive.Trigger>
