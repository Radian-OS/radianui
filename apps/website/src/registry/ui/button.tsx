...
const buttonVariants = cva(
  "inline-flex items-center justify-center box-border",
  {
    variants: {
      variant: {
        strong:
          "bg-primary font-semibold text-white hover:bg-primary-500 active:bg-primary-700",
        soft: "bg-primary/10 font-medium text-primary hover:bg-primary/15 active:bg-primary/20",
        outline:
          "bg-bg1 font-medium border border-primary text-primary hover:bg-primary/5 active:bg-primary/10",
        ghost:
          "bg-transparent text-primary font-medium hover:bg-primary/10 active:bg-primary/20",
        "neutral-soft": "bg-bg2 font-medium hover:bg-bg3 active:bg-bg4",
        "neutral-outline":
          "bg-bg1 font-medium border drop-shadow-xs hover:border-stroke-decorative active:bg-bg2 active:border-stroke-decorative",
      },
      rounded: {
        square: "rounded-xs",
        rounded: "rounded-md",
...
function Button({
  ...
  return (
    <button
      className={cn(
        buttonVariants({ variant, size, rounded, isIcon }),
        className,
      )}
      {...props}
    >
  ...
  const modifiedChildren = React.Children.map(children, (child, index) => {
    if (React.isValidElement(child)) {
      ...
      const borderRightClass =
        variant === "outline" || variant === "neutral-outline"
          ? "border-r"
          : "";
