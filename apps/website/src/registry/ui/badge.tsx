...
const badgeVariants = cva("inline-flex items-center font-medium box-border w-fit whitespace-nowrap", {
	variants: {
		variant: {
...
function Badge({ variant = "neutral-outline", size = "24", closable = false, className, children, ...props }: BadgeProps) {
	const [showBadge, setShowBadge] = useState(true)

	return (
		showBadge && (
			<div className={cn(badgeVariants({ variant, size }), "flex items-center gap-1", className)} {...props}>
				{Array.isArray(children)
					? children.map((child, index) =>
							typeof child === "object" && "type" in child && (child.type === "svg" || typeof child.type === "function") ? <span key={index}>{child}</span> : child
						)
					: children}
				{closable && (
					<X
