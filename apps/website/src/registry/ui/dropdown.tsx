  ...
  return (
    <DropdownMenuPrimitive.Portal>
      <DropdownMenuPrimitive.Content
        align="start"
        className={cn(
          "no-scrollbar border-stroke bg-bg1 z-50 min-w-[var(--radix-dropdown-menu-trigger-width)] overflow-x-visible overflow-y-scroll rounded-lg border px-1.5 py-1.5 drop-shadow-xs",
          className,
        )}
        sideOffset={8}
        {...props}
...
function DropdownItem({
  ...
  const { isSelectable, isSelected, handleSelect } =
    useDropdownSelection(value);
  ...
  return (
    ...
    >
      {asChild ? (
        ...
        <>
          {icon && <span>{icon}</span>}
          <span className="flex w-full gap-2 truncate font-normal">
            {children}
          </span>
          {shortcut && (
            <label
          ...
          {isSelectable && (
            <span className="flex h-2.5 w-3.5 items-center">
              {isSelected && <Check className="text-fg1" size={20} />}
            </span>
          )}
        </>
      )}
...
function DropdownGroup({
  ...
  React.useEffect(
    function () {
      if (selectedValues.length < minSelectionCount)
        console.warn(
          "minSelectionCount is greater than the number of selected keys",
        );
    },
    ...
    <>
      <DropdownCtx.Provider value={contextValue}>
        <DropdownMenuPrimitive.Group
          className={cn(
            className,
            "bg-bg1 z-50 flex flex-col items-stretch justify-start px-0 py-0",
          )}
          {...props}
        >
          {title && (
            <label className="text-fg3 flex h-7 items-center px-2 py-2.5 text-xs/4.5 font-medium uppercase">
              {title}
            </label>
          )}
          {children}
  ...
  return (
    <DropdownMenuPrimitive.Portal>
      <DropdownMenuPrimitive.SubContent
        className={cn(
          "border-stroke bg-bg1 z-50 flex min-w-36 flex-col items-stretch justify-start rounded-lg border p-1.5 drop-shadow-xs",
          className,
        )}
        sideOffset={10}
        alignOffset={-7}
