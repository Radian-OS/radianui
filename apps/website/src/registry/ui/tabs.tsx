...
const tabsListStyles = cva(
  "inline-flex h-10 items-center justify-start max-w-full overflow-x-scroll no-scrollbar",
  {
    variants: {
      size: {
        small: "",
...
function Tabs({
  ...
  return (
    <TabsVariantContext.Provider value={{ variant: variant, size: size }}>
      <TabsPrimitive.Root
        className={cn("no-scrollbar flex w-full flex-col gap-3", className)}
...
type PrimitiveTriggerExtended = React.ComponentPropsWithRef<
  ...
  return (
    <TabsPrimitive.Trigger
      className={cn(
        ...
        {
          "text-border-secondary text-fg3 pointer-events-none cursor-not-allowed":
            disabled,
        },
        className,
      )}
    ...
    >
      {icon != undefined && (
        <span
          className={cn("text-fg2 [&>svg]:size-5", { "text-fg3": disabled })}
        >
          {icon}
        </span>
      )}
