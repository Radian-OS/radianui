...
function convertToInternationalizedDate(
  ...
  if (selected && "from" in selected && "to" in selected) {
    return {
      ...
      to: selected?.to
        ? parseDate(format(selected.to, "yyyy-MM-dd"))
        : undefined,
    };
  }
...
export type CalendarProps = Omit<
  ...
  "selected" | "onSelect" | "numberOfMonths" | "disabled"
> &
...
function CalendarComponent({
  ...
  const mergedClassName = cn(
    "p-3 w-fit border border-stroke rounded-xl bg-bg1 drop-shadow-xs",
    className,
  );
  ...
  const mergedClassNames: Record<string, string> = {
    ...
    month_caption: cn(
      "mx-10 flex items-center justify-center z-20 p-0 text-sm font-semibold h-7",
      {
        hidden:
          props.hideNavigation ||
          hideCaption ||
          (navigatorStyle === "selector" && !doubleCalendar),
      },
    ),
    ...
    weekday:
      "text-fg3 body-sm font-medium size-8 shrink-0 flex items-center justify-center",
    ...
    button_previous: cn(
      "border rounded-lg border-stroke drop-shadow-xs p-1.5 flex justify-center items-center size-7",
      {
  ...
  if (navigatorStyle === "selector") {
    hideCaption = false;
    customComponents["Nav"] = () => (
      <SelectorNavigator
        localeCode={props.locale?.code as Intl.LocalesArgument}
      />
    );
  }
  ...
  const mergedComponents = {
    Chevron: (props: ChevronProps) => {
      if (props.orientation === "left")
        return <ChevronLeft size={16} className="stroke-fg1" />;
  ...
  function handleOnSelect(
    ...
    customOnSelect?.(
      convertedSelected as CalendarDate & CalendarDate[] & CalendarRange,
      convertedTriggerDate,
      modifiers,
      e,
    );
  }
...
function SelectorNavigator({
  ...
  const allMonths = Array.from({ length: 12 }, function (_, i) {
    return {
      name: new Intl.DateTimeFormat(localeCode, { month: "long" }).format(
        new Date(pickedYear, i),
      ),
