...
function Breadcrumb({
  ...
  const visibleItems = shouldShowEllipsis
    ? [childArray[0], ...childArray.slice(-maxItems + 1)]
    : childArray;
  const invisibleItems = shouldShowEllipsis
    ? childArray.slice(1, -maxItems + 1)
    : [];
            ...
            <React.Fragment key={child.key ?? index}>
              ...
              {showEllipsis && (
                <>
                      ...
                      <DropdownContent>
                        <DropdownGroup>
                          {invisibleItems.map((hiddenChild, hiddenIndex) =>
                            React.isValidElement<BreadcrumbItemProps>(
                              hiddenChild,
                            ) ? (
                              <DropdownItem
                                key={hiddenChild.key ?? hiddenIndex}
                              >
                                <a
                                  className="flex items-center justify-start gap-0.5"
                                  href={hiddenChild.props.href}
                                >
                  ...
                  {separatorType === "slash" ? (
                    <Slash size={14} className="stroke-fg3" />
                  ) : (
                    <ChevronRight size={14} className="stroke-fg3" />
                  )}
                </>
              )}
            </React.Fragment>
    ...
    <>
      <li className="flex items-center justify-center" {...props}>
        <Comp
          href={href}
          className={cn(
            "body-sm flex items-center gap-1 font-medium transition-colors",
            isCurrent ? "font-medium" : "text-fg2",
      ...
      {showSeparator && <SeparatorIcon size={14} className="stroke-fg3" />}
    </>
  );
}
