...
const selectTriggerVariants = cva(
	"r-select-trigger cn-select-trigger flex w-full items-center outline-none transition-shadow [&>span]:line-clamp-1",
	{
		variants: {
			size: {
				"28": "r-select-trigger-28 cn-select-trigger-28 gap-0.5",
				"32": "r-select-trigger-32 cn-select-trigger-32 gap-0.5",
				"36": "r-select-trigger-36 cn-select-trigger-36 gap-1",
				"40": "r-select-trigger-40 cn-select-trigger-40 gap-1",
				"44": "r-select-trigger-44 cn-select-trigger-44 gap-1",
				"48": "r-select-trigger-48 cn-select-trigger-48 gap-1",
			},
		},
		defaultVariants: {
			size: "36",
...
function SelectScrollUpButton({
	...
	return (
		<SelectPrimitive.ScrollUpButton
			...
			className={cn(
				"r-select-scroll-button cn-select-scroll-button flex cursor-default items-center justify-center",
				className
			)}
			{...props}>
	...
	return (
		<SelectPrimitive.Portal>
			<SelectPrimitive.Content
				...
				className={cn(
					"r-select-content cn-select-content data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 max-h-(--radix-select-content-available-height) origin-(--radix-select-content-transform-origin) relative z-50 overflow-y-auto overflow-x-hidden",
					position === "popper" &&
				...
				<SelectPrimitive.Viewport
					className={cn(
						"r-select-viewport cn-select-viewport",
						position === "popper" &&
...
function SelectLabel({ className, ...props }: SelectLabelProps) {
	return (
		<SelectPrimitive.Label
			...
			className={cn("r-select-label cn-select-label", className)}
			{...props}
		/>
	)
}
...
function SelectItem({ className, children, ...props }: SelectItemProps) {
	...
	return (
		<SelectPrimitive.Item
			...
			className={cn(
				"r-select-item cn-select-item outline-hidden relative flex w-full cursor-pointer select-none items-center",
...
function SelectDivider({ className, ...props }: SelectDividerProps) {
	return (
		<SelectPrimitive.Separator
			...
			className={cn(
				"r-select-divider cn-select-divider -mx-1.5 my-1.5 h-px",
				className
			)}
			{...props}
		/>
