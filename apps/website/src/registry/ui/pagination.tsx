...
const Pagination: React.FC<PaginationProps> = function ({
  ...
  const hiddenPagesBefore =
    startPage > 2 ? Array.from({ length: startPage - 2 }, (_, i) => i + 2) : [];
  const hiddenPagesAfter =
        ...
        pages.push(
          ...
          >
            <Select
              ...
              className="text-fg1 flex h-8 w-8 items-center justify-center bg-transparent"
            >
    ...
    for (let i = startPage; i <= endPage; i++) {
      pages.push(
        <Button
          variant={
            currentPage === i
              ? (buttonVariant as "strong" | "outline")
              : "neutral-outline"
          }
          key={i}
          onClick={function () {
    ...
    if (endPage < totalPages) {
      ...
      pages.push(
        <Button
          ...
          className="text-fg1 h-8 w-8 bg-transparent"
        >
          {totalPages}
        </Button>,
      );
      ...
      <div className="flex flex-col items-center gap-3 sm:flex-row">
        ...
        {enableCustomRows && (
          <span>
            ...
            {indexOfLastRow > totalPage ? totalPage : indexOfLastRow} of{" "}
            {totalPage} results
          </span>
        )}
      </div>
      <div className="flex gap-6">
        {rowPerPage && (
          <div className="flex items-center justify-center gap-6">
            ...
            <div>
              <Select
                selectedValues={[rowsPerPage.toString()]}
                onSelectedChange={handleRowsPerPageChange}
                className="h-8 w-max"
              >
            ...
            <div>
              <Select
                ...
                onSelectedChange={(values) =>
                  handlePageChange(parseInt(values[0], 10))
                }
                className="h-8 w-max"
              >
