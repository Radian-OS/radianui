...
Intl.supportedValuesOf("timeZone").map(function (zone) {
  timeZones[zone] = zone
    .split("/")
    .map((part) =>
      part.replace(/_/g, " ").replace(/(^|\s)\S/g, (t) => t.toUpperCase()),
    )
    .join("/");
});
const TIME_ZONES = Intl.supportedValuesOf("timeZone");

const DATE_RANGE_SHORTCUT_VALUES = [
  "today",
  "last_7_days",
  "last_30_days",
  "last_3_months",
...
function DatePicker({
  ...
  const [internalSelected, setInternalSelected] = React.useState<
    CalendarDate | CalendarDate[] | CalendarRange | undefined
  >(selected || undefined);
  ...
  const [selectedShortcut, setSelectedShortcut] = React.useState<string | null>(
    defaultDateRangeShortcutValue || null,
  );
  ...
  function onSelectHandler(
    ...
    onSelect?.(
      selected as CalendarDate & CalendarDate[] & CalendarRange,
      triggerDate,
      modifiers,
      e,
    );
  ...
  function isCalendarRange(
    value: CalendarDate | CalendarDate[] | CalendarRange,
  ...
  return (
    <Popover align="start">
        ...
        >
          <span className="w-full truncate text-left">
            ...
            {currentSelected &&
              ...
              format(
                currentSelected.toDate(getLocalTimeZone()),
                "MMM dd, yyyy",
              )}
            {currentSelected &&
              mode === "multiple" &&
              currentSelected instanceof Array &&
              (currentSelected as CalendarDate[])
                .map((date) =>
      ...
      <PopoverContent
        className={cn(
          "border-stroke bg-bg1 flex w-fit flex-col gap-3 rounded-xl border p-0 shadow-none drop-shadow-xs",
        )}
      >
        {mode === "single" && (
          <Calendar
        ...
        {mode === "range" && showDateRangeShortcut && (
          <div className="flex">
            <DateRangeShortcut
              ...
              selectedValue={selectedShortcut}
            />
            <Calendar
              mode="range"
              className="border-none drop-shadow-none"
...
function DateRangeShortcut({
  selectedValue,
  handleShortcutSelect,
}: DateRangeShortcutProps) {
  return (
    <div className="body-sm border-stroke text-fg1 flex flex-col border-r p-2 font-medium">
      {DATE_RANGE_SHORTCUT_VALUES.map((value) => (
        <DateRangeShortcutItem
          key={value}
          label={
            value.charAt(0).toUpperCase() + value.split("_").join(" ").slice(1)
          }
          value={value}
          onClick={function () {
...
function DateRangeShortcutItem({
  selectedValue,
  onClick,
  label,
  value,
}: DateRangeShortcutItemProps) {
  return (
    <span
    ...
    >
      {label}
      {selectedValue == value ? (
        <Check className="stroke-fg2" size={16} />
      ) : (
...
type TimePickerWrapperProps = Pick<
  TimePickerProps,
  "value" | "onValueChange"
> & {
...
function TimePickerWrapper({
  ...
  const allowedTimezones =
    timeZoneProps?.allowedTimezones || Object.keys(timeZones);
    ...
    <div className="flex w-full flex-1 gap-3 px-3 pb-3 shadow-none">
      ...
      <Select
        ...
        onSelectedChange={(values) =>
          values.length > 0 ? setTimezone(values[0]) : setTimezone(null)
        }
        {...timeZoneProps}
      >
