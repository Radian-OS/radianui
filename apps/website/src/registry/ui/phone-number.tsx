...
function PhoneNumber({
  ...
  const defaultSelected = FILTERED_COUNTRY_CODES.find(
    (data) => data.countryCode === defaultCountryCode,
  );
  const [internalCode, setInternalCode] = React.useState(
    defaultSelected?.code || "",
  ); /* Country Code */
  const [internalNumber, setInternalNumber] =
    React.useState(""); /* The number portion */
  ...
  function customTrigger(selectedValues: string[]) {
    const selectedCountry = FILTERED_COUNTRY_CODES.find(
      (data) => data.country == selectedValues[0],
    );
    return (
      <Button
        rounded={rounded}
  ...
  return (
    ...
    >
      <Select
        ...
        selectedValues={[
          FILTERED_COUNTRY_CODES.find((data) => data.code === internalCode)
            ?.country || "",
        ]}
        onSelectedChange={(values: string[]) => {
          setInternalCode(
            FILTERED_COUNTRY_CODES.find((data) => data.country === values[0])
              ?.code || "",
          );
        }}
