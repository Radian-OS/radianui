...
const TableFooter = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(function TableFooter(
	{ className, ...props },
	ref
) {
			...
			<DndContext id={useId()} collisionDetection={closestCenter} modifiers={[restrictToHorizontalAxis]} onDragEnd={handleDragEnd} sensors={sensors}>
				<div className={cn(`overflow-hidden ${dense && `rounded-md border`}`, tableHeight)}>
					<DragTable
						...
						<TableHeader className={`${sticky ? "bg-bg1 sticky top-0 z-10" : ""}`}>
		...
		<TableHead
			...
			{header.column.id !== "select" ? (
				<div className="flex items-center justify-start">
					{dragAndDrop && (
						<button className="hover:bg-bg2 -ml-2 rounded-md py-1 shadow-none" {...attributes} {...listeners} aria-label="Drag to reorder">
							<GripVertical className="text-fg2 h-4 opacity-60" strokeWidth={2} aria-hidden="true" />
						</button>
					)}
					...
					{enableSorting && (
						<button
							...
							{{
								...
								<ChevronUp className="h-4 shrink-0 opacity-0 group-hover:opacity-60" strokeWidth={2} aria-hidden="true" />
							)}
						</button>
					)}
				</div>
