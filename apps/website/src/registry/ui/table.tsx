...
const DragTable = React.forwardRef<
  HTMLTableElement,
  React.HTMLAttributes<HTMLTableElement>
>(function DragTable({ className, ...props }, ref) {
  return (
...
>(function TableFooter({ className, ...props }, ref) {
  return (
    <tfoot
      ref={ref}
      className={cn(
        "border-stroke bg-muted/50 border-t font-medium last:[&>tr]:border-b-0",
        className,
      )}
      {...props}
    />
...
>(function TableRow({ className, ...props }, ref) {
  return (
    <tr
      ref={ref}
      className={cn(
        "border-stroke hover:bg-muted/50 data-[state=selected]:bg-muted border-b transition-colors",
        className,
      )}
      {...props}
    />
...
>(function TableHead({ className, ...props }, ref) {
  return (
    <th
      ref={ref}
      className={cn(
        "text-muted-foreground px-3 py-2 text-left align-middle font-medium [&:has([role=checkbox])]:w-px [&>[role=checkbox]]:translate-y-0.5",
        className,
      )}
      {...props}
    />
...
>(function TableCell({ className, ...props }, ref) {
  return (
    <td
      ref={ref}
      className={cn(
        "px-3 py-2 align-middle [&>[role=checkbox]]:translate-y-0.5",
        className,
      )}
      {...props}
    />
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={cn(
            `overflow-hidden ${dense && `rounded-md border`}`,
            tableHeight,
          )}
        >
          <DragTable
          ...
          >
            <TableHeader
              className={`${sticky ? "bg-bg1 sticky top-0 z-10" : ""}`}
            >
              ...
              {table.getRowModel().rows?.length ? (
                table.getRowModel().rows.map((row, rowIndex) => (
                  <TableRow
                    ...
                    className={`${stripedTable && rowIndex % 2 === 0 ? "bg-bg2" : "hover:bg-bg2"} ${rowIndex % 2 !== 0 && stripedTable ? "hover:bg-transparent" : ""} `}
                    key={row.id}
                    data-state={row.getIsSelected() && "selected"}
                  >
                    {row
                      .getVisibleCells()
                      .map((cell: Cell<TData, unknown>, cellIndex) => {
...
const DraggableTableHeader = function <TData>({
  ...
  const { dragAndDrop, resizable, enableSorting, dense, verticalLine } =
    useTableContext();
  ...
  return (
    <TableHead
      ref={setNodeRef}
      className={`before:bg-border relative before:absolute before:inset-y-0 before:start-0 first:before:bg-transparent ${dense ? "bg-bg2" : ""} ${(verticalLine && headerGroup) || (resizable && headerGroup) ? "border-stroke border-r" : ""} `}
    ...
    >
      {header.column.id !== "select" ? (
        <div className="flex items-center justify-start">
          {dragAndDrop && (
            <button
              className="hover:bg-bg2 -ml-2 rounded-md py-1 shadow-none"
              {...attributes}
              {...listeners}
              aria-label="Drag to reorder"
            >
              <GripVertical
                className="text-fg2 h-4 opacity-60"
                strokeWidth={2}
                aria-hidden="true"
              />
            </button>
          )}
          <span className="text-fg2 z-10 ml-2 grow truncate">
          ...
          {enableSorting && (
            <button
              className="group text-fg2 -mr-1 bg-transparent py-0 shadow-none"
            ...
            >
              {{
                asc: (
                  <ChevronUp
                    className="h-4 shrink-0 opacity-60"
                    strokeWidth={2}
                    aria-hidden="true"
                  />
                ),
                ...
                <ChevronUp
                  className="h-4 shrink-0 opacity-0 group-hover:opacity-60"
                  strokeWidth={2}
                  aria-hidden="true"
                />
              )}
...
const DragAlongCell = function <TData>({
  ...
  return (
    <TableCell
      ref={setNodeRef}
      className={`truncate ${verticalLine && !isLastVisibleColumn ? "border-stroke border-r" : ""} `}
      style={style}
    >
