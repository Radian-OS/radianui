...
function NavItem({ item }: { item: (typeof NAV_ITEMS)[number] }) {
	...
	if (!item.children) {
		return (
			<div className="hover:bg-fill1-alpha group flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 transition-colors">
	...
	return (
		<Collapsible open={open} onOpenChange={setOpen}>
			<CollapsibleTrigger asChild>
				<div className="hover:bg-fill1-alpha flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 transition-colors">
					<Icon className="text-fg-secondary size-5 shrink-0" />
					<span className="text-fg flex-1 text-sm">{item.label}</span>
					{open ? (
