...
const NumberOfCalendarPreview = () => {
	const [mode, setMode] = useState<DatePickerModes>("range")
	const [totalCalendar, setTotalCalendar] = useState<number>(2)
		...
		<Tabs defaultValue="preview" variant={"outline-ghost"} size={"md"}>
			...
			<TabsContent value="preview">
				<div className={`flex h-[420px] flex-col ${totalCalendar === 3 ? "" : "items-center"} justify-center overflow-auto rounded-xl border px-10`}>
					{mode === "single" && <Calendar mode={"single"} numberOfMonths={totalCalendar} showOutsideDays />}
					{mode === "multiple" && <Calendar mode={"multiple"} numberOfMonths={totalCalendar} showOutsideDays />}
					{mode === "range" && <Calendar mode={"range"} numberOfMonths={totalCalendar} showOutsideDays />}
				</div>
			</TabsContent>
...
export default NumberOfCalendarPreview
