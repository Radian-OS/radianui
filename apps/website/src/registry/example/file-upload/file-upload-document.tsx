...
function getFileIcon(file: FileWithPreview) {
	const f = file.file
	const type = f instanceof File ? f.type : f.type
	const name = f instanceof File ? f.name : f.name
...
export default function FileUpload({ onContinue, onCancel }: FileUploadProps) {
	...
	useFileProgress(state.files, setProgressMap)
