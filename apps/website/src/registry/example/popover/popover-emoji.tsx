...
export default function PopoverEmoji() {
	...
	return (
		...
		// <EmojiPicker.Root className="bg-bg isolate flex h-[368px] w-fit flex-col">
		// 	<div className="flex items-center gap-2 px-3 py-2.5">
		// 		<Search className="size-4 shrink-0" />
		// 		<EmojiPicker.Search placeholder="Search" className="placeholder:text-fg-tertiary z-10 w-full appearance-none text-sm font-normal focus:outline-0" />
		// 	</div>
		// 	<EmojiPicker.Viewport className="outline-hidden relative flex-1">
		// 		<EmojiPicker.Loading className="absolute inset-0 flex items-center justify-center text-sm text-neutral-400 dark:text-neutral-500">Loading…</EmojiPicker.Loading>
		// 		<EmojiPicker.Empty className="absolute inset-0 flex items-center justify-center text-sm text-neutral-400 dark:text-neutral-500">No emoji found.</EmojiPicker.Empty>
		// 		<EmojiPicker.List
		// 			className="select-none pb-1.5"
		// 			components={{
		// 				CategoryHeader: ({ category, ...props }) => (
		// 					<div className="bg-white px-3 pb-1.5 pt-3 text-xs font-medium text-neutral-600 dark:bg-neutral-900 dark:text-neutral-400" {...props}>
		// 						{category.label}
		// 					</div>
		// 				),
		// 				Row: ({ children, ...props }) => (
		...
		// 				),
		// 				Emoji: ({ emoji, ...props }) => (
		// 					<button className="flex size-8 items-center justify-center rounded-md text-lg data-[active]:bg-neutral-100 dark:data-[active]:bg-neutral-800" {...props}>
		// 						{emoji.emoji}
		// 					</button>
		// 				),
		// 			}}
