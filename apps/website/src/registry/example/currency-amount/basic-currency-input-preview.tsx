...
const BasicCurrencyInputPreview = () => {
	...
	return (
		<div className="w-full max-w-sm space-y-2">
			<label className="text-fg text-sm font-medium">USD Currency (Large)</label>
			<CurrencyInputField
				size="44"
		...
		<Tabs defaultValue="preview">
			...
			<TabsContent value="preview">
				<div className="flex h-[420px] flex-col items-center justify-center rounded-xl border p-10">
					<div className="w-full max-w-sm space-y-2">
						<label className="text-fg text-sm font-medium">NPR Currency (Default)</label>
						<CurrencyInputField
							placeholder="Enter amount in NPR"
							value={value}
							onValueChange={(value: string | undefined) => setValue(value || "")}
							intlConfig={{ locale: "en-NP", currency: "NPR" }}
							prefix="Rs. "
						/>
						<p className="text-fg-tertiary text-xs">Value: {value || "0"}</p>
					</div>
			...
			<TabsContent value="code">
				<CodeSnippet title="basic-currency-input.tsx" showLineNumber className="h-[420px]" code={code} />
			</TabsContent>
		</Tabs>
	)
}
