...
const exampleMap = new Map<string, string>(Examples.flatMap((group: { name: string; files: { name: string; content: string }[] }) => group.files.map((f) => [f.name, f.content])))
...
function expandMdxComponents(rawMdx: string): string {
	...
	const expanded = rawMdx.replace(/<ComponentPreview\s+path="([^"]+)"[^/]*\/>/g, (_match, path: string) => {
...
function buildFrontmatter(doc: (typeof allDocs)[number]): string {
	...
	if (doc.externalSiteName) lines.push(`externalSiteName: ${doc.externalSiteName}`)
