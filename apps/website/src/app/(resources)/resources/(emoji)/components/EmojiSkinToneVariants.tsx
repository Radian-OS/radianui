		...
		<div className="grid grid-cols-2 items-stretch gap-3 sm:grid-cols-5">
			{variants.map((variant) => (
				<EmojiCopyButton
					...
					color="neutral"
					variant="outline"
					className="h-auto min-h-28 w-full min-w-0 flex-col gap-3 rounded-xl px-3 py-4"
					aria-label={`Copy ${variant.label} variant`}>
					<span className="text-4xl" aria-hidden="true">
						{variant.emoji}
					</span>
					<span className="text-fg-secondary flex min-h-8 items-center justify-center text-center text-xs font-medium whitespace-normal">
						{variant.label}
					</span>
				</EmojiCopyButton>
			))}
