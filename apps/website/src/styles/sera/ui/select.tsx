...
const selectTriggerVariants = cva(
	"border-border text-fg data-placeholder:text-fg-tertiary focus-visible:ring-primary-focus focus-visible:border-primary aria-invalid:border-error aria-invalid:ring-error [[data-invalid=true]_&]:border-error [[data-invalid=true]_&]:ring-error flex w-full items-center border-b bg-transparent shadow-none transition-shadow outline-none focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
	{
		variants: {
			size: {
				"28": "h-7 gap-0.5 rounded-none px-3 py-1.5 text-[13px]",
				"32": "h-8 gap-0.5 rounded-none px-3 py-1.5 text-sm",
				"36": "h-9 gap-1 rounded-none px-3.5 py-2 text-sm",
				"40": "h-10 gap-1 rounded-none px-4 py-2.5 text-sm",
				"44": "h-11 gap-1 rounded-none px-4 py-2.5 text-base",
				"48": "h-12 gap-1 rounded-none px-4.5 py-3 text-base",
			},
		},
		defaultVariants: {
			size: "36",
...
function SelectItem({ className, children, ...props }: SelectItemProps) {
	...
	return (
		<SelectPrimitive.Item
			...
			className={cn(
				"focus:bg-fill2-alpha relative flex w-full cursor-pointer items-center rounded-none py-2 text-sm outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50",
