...
const SRC_UTILITY_CSS: string = path.join(PROJECT_ROOT, "src/css/utility.css")
const DEST_DIR: string = path.join(PROJECT_ROOT, "public/css")
const DEST_CSS: string = path.join(DEST_DIR, "globals.css")
const DEST_UTILITY_CSS: string = path.join(DEST_DIR, "utility.css")
...
function generate(): void {
	...
	content = content.replace(
		/@custom-variant\s+style-(?:default|sera)\s+[^;]+;?(?:\r?\n)?/g,
		""
	)
	...
	if (fs.existsSync(SRC_UTILITY_CSS)) {
		fs.copyFileSync(SRC_UTILITY_CSS, DEST_UTILITY_CSS)
		console.log(` Generated: ${DEST_UTILITY_CSS}`)
	} else {
		console.warn(`  Utility CSS source file not found at ${SRC_UTILITY_CSS}`)
	}
}
