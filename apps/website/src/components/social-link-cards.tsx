...
const socialLinks = [
	{
		name: "Github",
		icon: (
			<svg fill="currentColor" className="fill-text-secondary size-5" viewBox="0 0 1.2 1.2" xmlns="http://www.w3.org/2000/svg">
					...
					<Link href={link.href} key={link.name} className="hover:bg-bg-level1 rounded-rounded-sm group cursor-pointer border p-4 transition-all duration-200 hover:shadow-md">
						...
						<div className="flex h-full items-center gap-3">
							...
							<div className="flex-1">
								<span className="text-text text-sm font-medium">{link.name}</span>
							</div>
						</div>
					</Link>
				)
