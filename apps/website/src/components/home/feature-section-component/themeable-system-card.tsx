...
export function ThemeableSystem({
	...
	const centerOffset =
		scrollIndex * ITEM_HEIGHT -
		(containerHeight / 2 - ITEM_HEIGHT / 2) +
		ACTIVE_OFFSET
	const centerOffsetX =
		scrollIndex * (ITEM_WIDTH + GAP_SIZE) -
	...
	return (
		<div
			ref={containerRef}
			className="relative flex h-full w-full select-none flex-col items-center gap-4 overflow-hidden px-10 sm:flex-row sm:gap-20">
				...
				{duplicatedData.map((colorOption, index) => {
					...
					return (
						<div
							...
							<Badge
								className={`min-w-30 ${isActive ? "bg-fill2" : ""}`}
								size="28"
								variant="outline"
								color="neutral">
								<BadgeDot
									className={
										COLOR_CLASSES[
											colorOption.value as keyof typeof COLOR_CLASSES
										]
									}
								/>{" "}
								<p className="w-full text-center">{colorOption.title}</p>
							</Badge>
						</div>
					)
				})}
...
function HoverCard({
	...
	return (
		<div
			onClick={onClick}
			className={`color-${activeColor} z-1 cursor-pointer`}>
			<div
				className={`border-border bg-bg flex h-fit w-80 flex-col overflow-hidden rounded-xl border p-0`}>
