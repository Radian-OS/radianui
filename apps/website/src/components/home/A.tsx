...
function A({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn(
        "border-stroke top-20 right-38 flex h-200 w-90 overflow-hidden rounded-2xl border transition-all duration-1000",
        className,
      )}
    >
      ...
      <div className="bg-bg1 flex w-full flex-col">
        <span className="w-full px-4 py-5 text-base leading-6 font-medium">
          My Workspace
        </span>
        <div className="text-fg2 flex flex-col gap-0.5 px-3">
          <span className="text-text-disabled px-2 py-2.25 text-xs font-medium">
            USER PANEL
          </span>
          <Item
            id="dashboard"
...
function Item({ id, icon, text, active, onClick }: ItemProps) {
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        "flex cursor-default items-center gap-3 rounded-md p-2 text-sm font-medium",
        { "bg-bg3": active, "hover:bg-bg2": !active },
      )}
    >
      {icon} <span>{text}</span>
    </div>
