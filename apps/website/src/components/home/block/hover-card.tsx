...
export default function HoverCard() {
	...
	return (
		<div
			...
			<div
				className={`border-border bg-bg flex w-80 flex-col overflow-hidden ${radiusBorderMap[radius]} border p-0`}>
				...
				<div
					...
					<div
						className={`flex h-5 ${spaceMap.gap3[spacing ?? "default"]} text-sm`}>
					...
					<div className={`flex ${spaceMap.gap3[spacing ?? "default"]}`}>
						<Button
							...
							className={`${radiusMap[radius]} ${colorMap[color] ?? ""} w-full flex-1 ${buttonStyles[button ?? "default"]}`}>
							Message
						</Button>
						<Button
							variant="outline"
							color="primary"
							size={sizeMap[size ?? "default"]}
							className={`${radiusMap[radius]} w-full flex-1`}>
							Follow
						</Button>
					</div>
				</div>
