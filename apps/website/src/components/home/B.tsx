    ...
    >
      <div className="flex flex-col">
        ...
        <div className="flex flex-col gap-0.5 px-3 py-1">
          <span className="text-text-disabled px-2 py-1 text-xs leading-4.5 font-medium">
            USER PANEL
          </span>
          <Item
            id="dashboard"
          ...
          <Accordion collapsible defaultValue="components">
            <AccordionItem value="components">
              <AccordionTrigger
                className={cn(
                  "hover:bg-bg2 flex h-10 cursor-default gap-3 rounded-md px-2 text-sm",
                )}
              >
                <Search className="text-fg2" />{" "}
              ...
              <AccordionContent className="pt-0.5">
                <div className="flex flex-col gap-0.5">
                  ...
                  <AccordionTextItem
                    text="Features"
                    active={activeAccordionItem === "Features"}
                    onClick={() => setActiveAccordionItem("Features")}
                  />
                  <AccordionTextItem
                    text="Testimonials"
          ...
          <Item
            ...
            text="Blocks"
            suffix={
              <span className="rounded-full bg-black px-1.5 py-0.5 text-xs text-white dark:bg-white dark:text-black">
                16
              </span>
            }
...
function AccordionTextItem({ text, active, onClick }: AccordionTextItemProps) {
  return (
    <span
      onClick={onClick}
      className={cn(
        "cursor-default rounded-md py-2 pr-2 pl-11 text-sm transition-colors",
        {
          "bg-bg3": active,
          "hover:bg-bg2": !active,
        },
...
function Item({ id, prefix, suffix, text, active, onClick }: ItemProps) {
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        "flex cursor-default items-center gap-3 rounded-md p-2 text-sm transition-colors",
        {
          "bg-bg3": active,
          "hover:bg-bg2": !active,
        },
