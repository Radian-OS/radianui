...
export function BackgroundCircles({ className, variant = "octonary", children }: BackgroundCirclesProps) {
		...
		<div className={cn("relative flex h-screen w-full items-center justify-center overflow-hidden", "bg-white dark:bg-black/5", className)}>
			<motion.div className="absolute h-[400px] max-h-[80vh] w-[400px] max-w-[80vw] tracking-tight">
				{[0, 1, 2].map((i) => (
					<motion.div
						key={i}
						className={cn("absolute inset-0 rounded-full", "border-2 bg-gradient-to-br to-transparent", variantStyles.border[i], variantStyles.gradient)}
						animate={{
							rotate: 360,
			...
			<motion.div className="relative z-10 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
				...
				<motion.p className="mt-6 text-lg text-slate-950 md:text-xl dark:text-white" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
					{description}
