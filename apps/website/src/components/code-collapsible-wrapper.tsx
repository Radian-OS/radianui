...
export function CodeCollapsibleWrapper({ className, children, ...props }: React.ComponentProps<typeof Collapsible>) {
	...
	return (
		<Collapsible open={isOpened} onOpenChange={setIsOpened} className={cn("group/collapsible relative", className)} {...props}>
			...
			<CollapsibleTrigger className="from-base/70 to-base text-fg-tertiary absolute inset-x-0 bottom-0 flex h-20 items-center justify-center rounded-b-lg bg-gradient-to-b text-sm group-data-[state=open]/collapsible:hidden">
				{isOpened ? "Show Less" : "Show More"}
			</CollapsibleTrigger>
		</Collapsible>
	)
