		...
		return (
			<AccordionPrimitive.Item
				...
				<AccordionPrimitive.Trigger
					className={cn(
						`flex w-full items-center gap-1 rounded-md p-2 text-sm`,
						className,
						{
				...
				<AccordionPrimitive.Content className="data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down relative h-full w-full overflow-hidden text-sm">
					{element && indicator && <TreeIndicator aria-hidden="true" />}
					<AccordionPrimitive.Root
						dir={direction}
						type="multiple"
						className="ml-5 flex w-full flex-col"
	...
	(
		...
		return (
			<button
				...
				disabled={!isSelectable}
				className={cn(
					"hover:bg-fill2 flex w-full items-center gap-1 rounded-md p-2 text-sm duration-200 ease-in-out",
					{
				...
				{...props}>
				{fileIcon ?? <FileCode2 className="size-4" />}
				{children}
			</button>
		)
	}
