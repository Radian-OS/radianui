...
export async function fetchAllCategories(): Promise<string[]> {
  try {
    const apiUrl =
      "https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyBqC0iPLWLeOsKyhlmriYgRx_Sy4lIgE4Q";
    ...
    const categories = Array.from(
      new Set(
        data.items
          .map((font: GoogleFont) => font.category)
          .filter(
...
export async function fetchFontsByCategory(
  category: string,
): Promise<GoogleFont[]> {
  try {
    const apiUrl =
    ...
    const data = (await response.json()) as GoogleFontResponse;
    const fontsByCategory = data.items.filter(
