				...
				<>
					<div
						...
						<div className="flex items-center justify-center gap-2 text-sm/5">
							...
							{icon && (
								<div className={`${React.isValidElement(icon) && icon.props?.className ? "" : "h-5 w-5"} flex items-center justify-center`}>
									{icon}
								</div>
							)}
...
export const Toaster = function ({ bgColor = "default", ...props }: ToasterProps) {
	return (
		<Sonner
			...
			toastOptions={{
				classNames: {
					toast: toastClass({ bgColor: bgColor as VariantToasterType, position: props.position?.charAt(0) === "b" ? "bottom" : "top" }),
				},
			}}
			{...props}
		/>
