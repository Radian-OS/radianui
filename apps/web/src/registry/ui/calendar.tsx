...
function convertToNativeDate(
	...
	if (selected && "from" in selected && "to" in selected) {
		return { from: selected.from.toDate(timeZone), to: selected?.to?.toDate(timeZone) }
	}
...
function SelectorNavigator({
	...
	const allMonths = Array.from({ length: 12 }, function (_, i) {
		return { name: new Intl.DateTimeFormat(localeCode, { month: "long" }).format(new Date(pickedYear, i)), date: new Date(pickedYear, i) }
	})
