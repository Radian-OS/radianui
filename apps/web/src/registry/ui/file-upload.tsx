...
function FileUpload({
	...
	async function upload(file: File) {
		...
		return new Promise<void>(function () {
			...
			setUploadStatuses((prev) => ({ ...prev, [fileIdentifier]: { progress: 0, status: "uploading" } }))
			...
			xhr.upload.addEventListener("progress", function (event) {
				if (event.lengthComputable) {
					...
					setUploadStatuses((prev) => ({ ...prev, [fileIdentifier]: { ...prev[fileIdentifier], progress } }))
				}
			})

			xhr.onload = function () {
				if (xhr.status >= 200 && xhr.status < 300) {
					setUploadStatuses((prev) => ({ ...prev, [fileIdentifier]: { progress: 100, status: "success" } }))
				} else {
					setUploadStatuses((prev) => ({ ...prev, [fileIdentifier]: { ...prev[fileIdentifier], status: "error" } }))
				}
			}
	...
	return (
		<div className={cn("flex w-90 flex-col gap-2", containerClassName)}>
			{label && <label className={cn("body-sm w-fit font-medium", { "text-fg3": props.disabled })}>{label}</label>}
				...
				<div
					...
					<div className="flex flex-col items-center justify-center gap-4">
						...
						<div className="text-muted-foreground flex flex-col text-sm">
							<span className={cn("body-sm text-fg1 font-medium", { "text-fg3": props.disabled })}>Upload an image or video</span>
							<span className={cn("body-xs text-fg2 font-normal", { "text-fg3": props.disabled })}>
