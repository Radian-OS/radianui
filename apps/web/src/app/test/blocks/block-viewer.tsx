...
export const BlockPreview: React.FC<BlockPreviewProps> = ({ code, preview, title, category, previewOnly }) => {
	...
	const { copied, copy } = useCopyPaste({ code: code as string, title, category, eventName: "block_copy" })
	const { copied: cliCopied, copy: cliCopy } = useCopyPaste({ code: terminalCode, title, category, eventName: "block_cli_copy" })
				...
				<div className="z-10 mx-auto flex max-w-7xl justify-between py-1.5 pr-6 pl-8">
					<div className="-ml-3 flex items-center gap-3">
						...
						<span className="text-muted-foreground hidden text-sm lg:block">
							{width < MDSIZE ? "Mobile" : width < LGSIZE ? "Tablet" : "Desktop"}
						</span>{" "}
					</div>
